...
async function main() {
  const app = initializeApp(FIREBASE_CONFIG, `lungs-backfill-last-measurement-${Date.now()}`);
  ...
  console.log(`Lungs last-measurement backfill ${WRITE_MODE ? 'WRITE' : 'DRY RUN'}`);
  ...
  writeJson(path.join(OUTPUT_DIR, `lungs-backfill-last-measurement-plan-${timestamp}.json`), {
